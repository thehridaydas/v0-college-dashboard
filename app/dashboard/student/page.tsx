...
export default async function StudentDashboardPage() {
  ...
  const student = await db.student.findUnique({
    ...
    include: {
      enrollments: {
        include: {
          class: { include: { course: true, subjects: true } },
        },
        take: 1,
      },
    },
  ...
  return (
    <StudentOverview
      ...
      attendanceStats={{ attendancePct, present: presentCount, late: lateCount, absent: attendanceStats.find((a) => a.status === "ABSENT")?._count.status ?? 0, total: totalAttendance }}
